...
function parseCategory(
  ...
  const weightClass = weightMatch
    ? weightMatch[0].replace(/\s/g, "")
    : "open";
...
function parsePlacement(line: string): ParsedPlacement | null {
  const match = line.match(
    /^\s*(\d+)\s*([A-ZÄÖÜÉÈÀÂÊÎÔÛÇ\s'-]+(?:\s*[+-]?\d+)?)\s*-\s*(\w+)\s*$/i
  );
  ...
  return {
    ...
    name: match[2].trim().replace(/\s*[+-]?\d+\s*$/, "").trim(),
    country: match[3].trim(),
  };
}

export async function parseStandingsPdf(
  pdfBuffer: Buffer
): Promise<ParsedTournamentResults> {
  const pdfParse =
    (await import("pdf-parse-debugging-disabled")).default;
